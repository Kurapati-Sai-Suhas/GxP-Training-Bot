...
- If an **Adaptive Retraining Due** card is showing, lead with it: "this SOP is due for a spaced retest under our scheduler — a real Leitner-style algorithm, not a static suggestion." (It won't show for a freshly seeded learner with no prior attempts — spaced intervals mean nothing is "due" until real time has passed since their last attempt, which is correct behavior, not a bug. If you need to show it live, mention that it's driven by the same signal that already updates on every quiz submission.)
- If a **Recommended Refresher** card is also showing, point it out too: "this is computed from this learner's own past wrong answers — a simpler, complementary signal to the scheduler above."
...
| "Do you have tests?" | Yes — 51 backend tests (accounts, sops, ai_engine, quiz, attempts, analytics, audit), including RBAC boundary tests per role, electronic-signature boundary tests, adaptive-retraining scheduling tests, a forced offline-fallback path for AI generation so tests never need a live API key, and regression tests for real bugs we found and fixed during development. CI runs the suite against a real Postgres service container. |
| "How does the adaptive retraining actually decide when to re-test someone?" | A Leitner-style box scheduler: a correct answer pushes the next eligible retest date out (1 → 2 → 4 → 7 → 14 → 30 days), a wrong answer resets it to tomorrow. Three correct in a row with no miss in between marks the topic "mastered" and it stops being surfaced. We deliberately chose this over Bayesian/deep knowledge tracing — those need far more response data per skill than a deployment this size will ever generate; see the roadmap for the literature behind that call. |
