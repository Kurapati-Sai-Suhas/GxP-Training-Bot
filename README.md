...
flowchart TD
    ...
    O --> P[TopicMastery updated<br/>Leitner-style spaced repetition]
...
- **Ye, Su & Cao**, *"A Stochastic Shortest Path Algorithm for Optimizing Spaced Repetition Scheduling,"* KDD 2022, and **Settles & Meeder**, *"A Trainable Spaced Repetition Model for Language Learning,"* ACL 2016 (the model behind Duolingo's Half-Life Regression) — both make the case against treating every quiz item as equally hard; this project's `DIFFICULTY_WEIGHT` (easy/medium/hard mastery weighting in `attempts/views.py`) is a lightweight, data-free version of that same idea.
...
Every completed attempt updates `TopicMastery` (one row per learner+SOP): correct → advance a Leitner box (review interval expands: 1 → 2 → 4 → 7 → 14 → 30 days) and increment a streak; wrong → reset to box 0. Three correct in a row → `mastered`, and the topic stops being surfaced. `GET /api/attempts/auto-assigned/` doesn't just suggest a due topic — it **pre-creates the actual `QuizAttempt`** (idempotently — reloading the page doesn't spawn duplicates) and targets it at the specific questions the learner previously got wrong, falling back to the full approved set if there's no miss history yet.
...
- **65 automated tests** (`accounts`, `sops`, `ai_engine`, `quiz`, `attempts`, `analytics`, `audit`) — RBAC boundary tests from *both* the permitted and denied side of every gated action, electronic-signature boundary tests (missing password / wrong password), forced-offline-fallback tests for both AI generation and the SOP chatbot (CI never needs a live NVIDIA API key), and adaptive-retraining scheduling tests covering box advancement, streak resets, difficulty weighting, and low-confidence-question exclusion.
...
See [`ROADMAP.md`](ROADMAP.md) and Section 11 of [`docs/SRS_GxP_Training_Bot.docx`](docs/SRS_GxP_Training_Bot.docx) for the full production-readiness roadmap (secrets management, TLS, rate limiting, observability, horizontal scaling) and a designed-but-not-yet-built n8n workflow-automation integration. The two nearest open items in the codebase itself: a frontend test suite (backend has 65 tests, frontend has none yet), and multi-turn memory for the SOP chatbot (today it's single-turn: one question, one grounded answer).
